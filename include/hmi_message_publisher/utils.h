...
static void writeDataToDataVector(std::vector<unsigned char>& data_buffer,
                                  const unsigned char& start_pos,
                                  const unsigned char& n,
                                  const unsigned int& data) {
    size_t end_pos = start_pos + n;
    if (data_buffer.size() * 8 < end_pos) {
        LOG(ERROR) << "Trying to write data from " << (int)start_pos << " to "
                   << (int)(start_pos + n)
                   << " : larger then data_buffer size: " << data_buffer.size() * 8;
        return;
    }
...
static void getDataFromDataVector(const std::vector<unsigned char>& data_buffer,
                                  const unsigned char& start_pos,
                                  const unsigned char& n,
                                  unsigned int& data) {
    size_t end_pos = start_pos + n;
    if (data_buffer.size() * 8 < end_pos) {
        LOG(ERROR) << "Trying to get data from " << (int)start_pos << " to " << (int)(start_pos + n)
                   << " : larger then data_buffer size: " << data_buffer.size() * 8;
        return;
    }
    data = 0;
 ...
 * put(key): put the key in the map and automatically assigned a least used value( in the range of 0
 * to _cacheCapacity) to it.
 */
class LRUCache {
  public:
    LRUCache(const unsigned int capacity) : _cacheCapacity(capacity) {}
    ...
    int put(int key) {
        ...
        if (_map.find(key) != _map.end()) {
            ...
            if (_map.size() < _cacheCapacity) {
                ...
                // map is at capacity limit, so remove the last (LRU) entry in cache list, and reuse
                // it's value
