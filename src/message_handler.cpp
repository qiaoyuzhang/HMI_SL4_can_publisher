...
MessageHandler::MessageHandler() : _hmi_ob_id_cache(hmi_message::ObstacleExtendedInfo_ob_id.max) {}

void MessageHandler::handleSteeringReport(const dbw_mkz_msgs::SteeringReport::ConstPtr& msg) {
    DLOG(INFO) << "handleSteeringReport";
    DLOG(INFO) << "speed : " << ConvertSpeedUnitFromMps(msg->speed) << " " << _speed_unit
...
void MessageHandler::handleDbwEnabledUpdate(const std_msgs::Bool::ConstPtr& msg) {
    ...
    if (msg->data) {
        _data_buffer.vehicle_status_general_info.setEngageStatus(
                hmi_message::VehicleEngageStatus::ENGAGED);
    } else {
        _data_buffer.vehicle_status_general_info.setEngageStatus(
                hmi_message::VehicleEngageStatus::READY);
    }
}
...
void MessageHandler::handleCheckStatus(const std_msgs::String::ConstPtr& msg) {
    ...
    if (_data_buffer.vehicle_status_general_info.getEngageStatus() ==
        hmi_message::VehicleEngageStatus::ENGAGED) {
        DLOG(INFO) << "ENGAGED";
        return;
    }
    ...
    if (check_status_str.find("\"sick\": false") != std::string::npos) {
        DLOG(INFO) << "NOT READY";
        _data_buffer.vehicle_status_general_info.setEngageStatus(
                hmi_message::VehicleEngageStatus::NOT_READY);
    } else {
        DLOG(INFO) << "READY";
        _data_buffer.vehicle_status_general_info.setEngageStatus(
                hmi_message::VehicleEngageStatus::READY);
...
void MessageHandler::handleTurnSignalCmd(const dbw_mkz_msgs::TurnSignalCmd::ConstPtr& msg) {
    ...
    _data_buffer.planning_general_info.setLeftLaneChangeCmd(msg->cmd.value ==
                                                            dbw_mkz_msgs::TurnSignal::LEFT);
    _data_buffer.planning_general_info.setRightLaneChangeCmd(msg->cmd.value ==
                                                             dbw_mkz_msgs::TurnSignal::RIGHT);
}
...
void MessageHandler::handlePlanningTrajectory(const std_msgs::String::ConstPtr& msg) {
    ...
    if (map_interface != nullptr and map_interface->StatefulLocate(ego_xy, current_map_node) and
        ...
        DLOG(INFO) << "get speed_limit from map : " << ConvertSpeedUnitFromMps(road_speed_limit_mps)
                   << " " << _speed_unit;
        _data_buffer.planning_general_info.setSpeedLimit(
                ConvertSpeedUnitFromMps(road_speed_limit_mps));
    } else {
        DLOG(INFO) << "can't get speed limit from map, so use previous speed_limit: "
                   << _data_buffer.planning_general_info.getSpeedLimit() << " " << _speed_unit;
    }
}
...
void MessageHandler::handleObstacles(const std_msgs::String::ConstPtr& msg) {
    ...
    for (const auto& obstacle : obstacle_detection.obstacle()) {
        ...
        ConvertWorld2IMU(_data_buffer.pose,
                         imu_point,
                         obstacle.motion().x(),
                         obstacle.motion().y(),
                         obstacle.motion().z());
        ...
        if (_data_buffer.active_AEB) {
            if (obstacle.ego_lane_presence() ==
                drive::common::perception::PerceptionObstacle::IN_EGOLANE_LEAD_VEHICLE) {
        ...
        hmi_message::ObstacleExtendedInfo obstacle_extended_info(
                hmi_ob_id, lane_assignment, imu_point[0], y, obstacle.type(), is_threat);
...
void MessageHandler::ConvertWorld2IMU(const Pose& pose,
                                      std::vector<double>& imu_point,
