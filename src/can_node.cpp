...
CanNode::CanNode(const int index)
    : can_(index), status_reporter_(drive::common::monitor::StatusReporter::getInstance()) {
    init();
}
...
void CanNode::init() {
    ...
    timer_write_to_can_ =
            node_.createTimer(ros::Duration(0.1), &CanNode::writeDataToCan, (CanNode*)this);

    // set timer to publish status report to can every 1 s
    timer_publish_status_report_ =
            node_.createTimer(ros::Duration(1), &CanNode::publishStatusReport, (CanNode*)this);
    ...
    turn_signal_cmd_sub_ = node_.subscribe(
            turn_signal_cmd_topic, 10, &MessageHandler::handleTurnSignalCmd, (MessageHandler*)this);
...
void CanNode::writeDataToCan(const ros::TimerEvent& event) {
    ...
    can_.WriteToCan(hmi_message::VehicleStatusGeneralInfoMsgId,
                    _data_buffer.vehicle_status_general_info.getVectorData());
    can_.WriteToCan(hmi_message::ObstacleGeneralInfoMsgId,
                    _data_buffer.obstacle_general_info.getVectorData());
    ...
    can_.WriteToCan(hmi_message::LaneGeneralInfoMsgId,
                    _data_buffer.lane_general_info.getVectorData());
    ros::Duration(0, 1).sleep();
    can_.WriteToCan(hmi_message::PlanningGeneralInfoMsgId,
                    _data_buffer.planning_general_info.getVectorData());
}
