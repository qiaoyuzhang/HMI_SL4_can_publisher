...
TEST(PlanningGeneralInfo, initialization) {
    ...
    PlanningGeneralInfo planning_msg(false, false, true, 2, 55.5);
    ...
    ASSERT_NEAR(planning_msg2.getSpeedLimit(), 55.5 , PlanningGeneralInfo_speed_limit.resolution/2.0); 
   
}

TEST(PlanningGeneralInfo, set_value) {
    
    PlanningGeneralInfo planning_msg(false, false, false, 2, 55.5);
    
    planning_msg.setAllowLaneChange(true);
    planning_msg.setLeftLaneChangeCmd(true);
    planning_msg.setRightLaneChangeCmd(true);
    
    planning_msg.setSetSpeed(50.2);
    ...
    ASSERT_EQ(planning_msg.getRightLaneChangeCmd(),true);
    ASSERT_NEAR(planning_msg.getSetSpeed(), 50.2 , PlanningGeneralInfo_set_speed.resolution/2.0); 
