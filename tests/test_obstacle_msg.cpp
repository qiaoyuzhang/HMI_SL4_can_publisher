...
TEST(ObstacleExtendedInfo, initialization) {
    ObstacleExtendedInfo obstacle_msg(10, 2, 0.5, -1.2, 2, false);
    ASSERT_EQ(obstacle_msg.getObId(), 10);
    ASSERT_EQ(obstacle_msg.getLaneAssignment(), 2);
    ASSERT_NEAR(obstacle_msg.getDistLong(), 0.5, ObstacleExtendedInfo_dist_long.resolution / 2.0);
    ASSERT_NEAR(obstacle_msg.getDistLat(), -1.2, ObstacleExtendedInfo_dist_lat.resolution / 2.0);
    ...
    ASSERT_NEAR(obstacle_msg2.getDistLat(), -1.2, ObstacleExtendedInfo_dist_lat.resolution / 2.0);
    ASSERT_EQ(obstacle_msg2.getObThreat(), false);
}

TEST(ObstacleExtendedInfo, set_value) {
    ObstacleExtendedInfo obstacle_msg(10, 2, 0, 100, 2, false);

    obstacle_msg.setObId(155);
    ASSERT_EQ(obstacle_msg.getObId(), 155);

    obstacle_msg.setLaneAssignment(3);
    ASSERT_EQ(obstacle_msg.getLaneAssignment(), 3);

    obstacle_msg.setDistLong(ObstacleExtendedInfo_dist_long.max +
                             3 * ObstacleExtendedInfo_dist_long.resolution);
    ASSERT_EQ(obstacle_msg.getDistLong(), ObstacleExtendedInfo_dist_long.max);

    obstacle_msg.setDistLong(ObstacleExtendedInfo_dist_long.offset -
                             3 * ObstacleExtendedInfo_dist_long.resolution);
    ...
    std::string name =
            PerceptionObstacle_descriptor->FindValueByNumber(obstacle_msg.getObClass())->name();
    ASSERT_EQ(name, "CAR");
